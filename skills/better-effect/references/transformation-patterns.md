...
Quando failures transitórias já estão tipadas:
...
requestEffect.retry(
  ...
  whileError: (failure) => failure is TemporaryFailure,
);
```

Não transforme defects em failures apenas para poder fazer retry.

---

## `Future.wait` ilimitado -> `Effect.forEach` com limite

### Antes

```dart
final users = await Future.wait(ids.map(repository.load));
```
...
Escolha o limite a partir da capacidade real do sistema. Use unbounded apenas deliberadamente.

---

## Contexto temporário em singleton -> Module por execução

Se request/tenant/transaction está sendo colocado em singleton mutável para ficar acessível em várias camadas, considere um Module local executado por `runWith`/`runExitWith`/`executeWith`.

Não use essa transformação para argumentos normais de domínio.
