...
extension BetterEffectGraphQueries on BetterEffectGraph {
  ...
  BetterEffectGraphService resolveService(String selector) {
    ...
    if (matches.isEmpty) {
      final available =
          services.map((service) => service.selector).toSet().toList()..sort();
      throw BetterEffectGraphSelectionException(
  ...
  BetterEffectModuleExplanation explainModule(String selector) {
    ...
    final providedServices = providers
    ...
    final relatedDiagnostics =
        diagnostics
            .where(
              (diagnostic) => relatedPaths.contains(diagnostic.location.path),
            )
            .toList()
  ...
  List<BetterEffectDependencyPath> whyService(
    ...
    paths.sort((left, right) {
      ...
      return left.serviceIds
          .join('\u0000')
          .compareTo(right.serviceIds.join('\u0000'));
    });
  ...
  List<BetterEffectDependencyPath> _whyInModule(
    ...
    while (queue.isNotEmpty) {
      ...
      if (current == targetServiceId) {
        ...
        paths.add(
          BetterEffectDependencyPath(moduleId: module.id, serviceIds: path),
        );
        continue;
      }
