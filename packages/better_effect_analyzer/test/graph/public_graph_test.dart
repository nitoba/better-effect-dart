...
void main() {
  ...
  test('graph JSON has a stable schema and relative locations', () async {
    ...
    final firstLocation =
        (modules.first as Map<String, Object?>)['location']
  ...
  test(
    'explain shows effective providers and root fallback requirements',
    () async {
      final graph = await BetterEffectGraphChecker(app.path).graph();
  ...
  test(
    'why returns deterministic shortest paths across selected roots',
    () async {
      final graph = await BetterEffectGraphChecker(app.path).graph();
  ...
  test(
    'unused reports only declarations unreachable from complete roots',
    () async {
      final graph = await BetterEffectGraphChecker(app.path).graph();
      final unused = graph.unusedDeclarations();

      expect(unused.modules.map((module) => module.name), <String>[
        'orphanModule',
      ]);
      expect(
  ...
  test('renders JSON, DOT, Mermaid and SARIF deterministically', () async {
    ...
    expect(
      (jsonDecode(json) as Map<String, Object?>)['schemaVersion'],
    ...
    jsonEncode(<String, Object>{
      ...
      'packages': <Object>[
        ...
        <String, Object>{
          'name': 'better_effect',
          'rootUri': p
              .toUri(p.join(temporary.path, 'better_effect'))
              .toString(),
          'packageUri': 'lib/',
          'languageVersion': '3.10',
        },
...
environment:
  sdk: ^3.10.0
''');
  final library = File(p.join(package.path, 'lib', 'better_effect.dart'))
